...
def train(x, y, xtest, ytest, net):
	...
	loss_0 = loss_func(prediction[:,0], ytest[:,0])
	loss_1 = loss_func(prediction[:,1], ytest[:,1])
	loss_2 = loss_func(prediction[:,2], ytest[:,2])

	loss = loss_0 + loss_1 + loss_2
	print('Validation loss {0}'.format([loss_0, loss_1])) 
	print('Validation r2score {0}'.format(r2_score(ytest[:,0].data.numpy(), prediction[:, 0].data.numpy())))
	print('Validation r2score {0}'.format(r2_score(ytest[:,1].data.numpy(), prediction[:, 1].data.numpy())))
...
def regression(X, Y):
	...
	for train_index, test_index in kf.split(X):
		...
		lss, r2s = train(X_train, Y_train, X_test, Y_test, net)
