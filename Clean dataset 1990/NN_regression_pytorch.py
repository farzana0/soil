...
def regplot(x, y, loss, r2score, target):
	...
	ax.set_title('NN ' +  target  + ', loss = ' + "{:.2f}".format(loss) + ', R2 = ' + str(r2score))
...
def train(x, y, xtest, ytest, net):
	...
	# net = Net(n_feature= num_features, n_hidden=20, n_output=num_output)     # define the network
	# optimizer = torch.optim.Adam(net.parameters(), lr=learning_rate, weight_decay=0.1)
	# loss_func = torch.nn.MSELoss()  # this is for regression mean squared loss
	...
	r2score = r2_score(ytest.data.numpy(), prediction.data.numpy())
	print('Validation r2score {0}'.format(r2score))
	r2score = r2_score(ytest.data.numpy(), prediction.data.numpy())
	return loss.data.numpy(), r2score, prediction.data.numpy(), ytest.data.numpy()


def regression(X, Y, target):
	# write down the target and adjust for the regplot function
	X = normalizedata(X)
	...
	for train_index, test_index in kf.split(X):
		...
		lss, r2s, prediction, ytest = train(X_train, Y_train, X_test, Y_test, net)
		# regplot(ytest, prediction, float(lss), r2s, target)
		r2sc.append(float(r2s))
		loss_.append(float(lss))
	regplot(ytest, prediction, float(lss), r2s, target)

	prediction = net(Variable(torch.from_numpy(X).type(torch.FloatTensor)))
	# regplot(Y, prediction.data.numpy(), float(lss), r2s, target)
	...
	return prediction.data.numpy(),  statistics.mean(loss_),  statistics.mean(r2sc)      
